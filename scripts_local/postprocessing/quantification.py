...
#%% plot the analysis bar plot - separate years
...
# plotting
# percent_galapnj_year_df.plot.bar(width=0.7,ax=ax,legend=False)
# percent_galahc_year_df.plot.bar(width=0.7,ax=ax2,legend=False)

percent_gala_year_df.loc[:,sources[:9]].plot.bar(width=0.7,ax=ax,legend=False)
percent_gala_year_df.loc[:,sources[9:]].plot.bar(width=0.7,ax=ax2,legend=False)

# ax for PNJ
ax.set_title('Contribution',fontsize=20)
ax.set_xlim([-0.5,13.5])
ax.set_ylim([-105,40])
# ax.set_yticks(np.arange(0,101,20))
ax.axhline(y=0.,c='k',linewidth=2,zorder=9)
# ax.bar_label(ax.containers[0], fmt='%.2f%%', padding=5, fontsize=14)
ax.set_xlabel('Years',**lbm)
ax.set_ylabel('Representation',labelpad=12,**lbm)
...
ax2.axhline(y=0.,c='k',linewidth=2,zorder=9)
...
# legend
ax.legend(**lbm,loc='upper right',bbox_to_anchor=(1.25,0.85))

plt.tight_layout()
# plt.savefig(pltdir+f'representation2.png')

#%% plot the analysis bar plot - all years
ttl = dict(fontsize=28,pad=12)
lbm = dict(fontsize=22)
tkm = dict(labelsize=20)
...
# All pass the PNJ band
ax = fig.add_subplot(gs[0])

percent_galapnj_df.plot.bar(width=0.7,ax=ax,rot=45,color=color,legend=False)

ax.legend(loc='upper right', bbox_to_anchor=(1.,1.),**lbm)
ax.set_title(f'The fraction of Galapagos particles that are transported by PNJ',**ttl)
ax.set_ylim([0,100])
...
# plotting
percent_total_df.plot.barh(width=0.7,ax=ax,color=['tab:blue','tab:orange'],stacked=True)
# percent_galahc_df.plot.barh(width=0.7,ax=ax2,color='tab:blue',legend=False)
# percent_galapnj_df.plot.barh(width=0.7,ax=ax,color='tab:orange',legend=False)

# ax for posterior
ax.invert_yaxis()
ax.set_title('The fraction of particles transported by different ocean dynamics',**ttl)
ax.set_xlim([0,100])
ax.set_xticks(np.arange(0,101,20))
# ax.bar_label(ax.containers[0], fmt='%.2f%%', label_type='edge', padding=10, fontsize=14)
# ax.bar_label(ax.containers[1], fmt='%.2f%%', xy=5, fontsize=14)
ax.set_xlabel('Fraction [%]',**lbm)
ax.set_ylabel('Sources (from north to south)',labelpad=16,**lbm)
ax.tick_params(axis='both',**tkm)
ax.legend(**lbm)
ax.grid(**grdl)
ax.arrow(-14,1,0,10,clip_on=False,width=0.1,head_length=0.8,head_width=1.5,length_includes_head=True,color='k')

plt.tight_layout()
plt.savefig(pltdir+f'quantification_attribution.png')
...
#%% N5N2 vs N7N2 - Gala or not
region = ['PNJtilt','PNJtilt_N7N2']
loadyears = np.arange(2007,2021)
percentT_df = {}
...
for i,r in enumerate(region):
    ax = fig.add_subplot(gs[i])
    percentT_df[r] = calculate(r, loadyears)[0]
    percentT_df[r].transpose().plot.bar(width=0.7,ax=ax,rot=45,color=color,legend=False)
    
    ax.legend(loc='upper right', bbox_to_anchor=(1.1,1.))
    ax.set_title(f'The fraction of tube-transported parcels end up in Galapagos - {r}',fontsize=16)
    ax.set_ylim([0,100])
    ax.set_ylabel('fraction [%]',fontsize=12)
    ax.grid()

ax = fig.add_subplot(gs[2])
improve = percentT_df['N5N2'] - percentT_df['N7N2']
improve.transpose().plot.bar(width=0.7,ax=ax,rot=45,color=color,legend=False)

ax.legend(loc='upper right', bbox_to_anchor=(1.1,1.))
ax.set_title(f'The fraction of tube-transported parcels end up in Galapagos',fontsize=16)
ax.set_ylim([-50,50])
ax.set_ylabel('fraction [%]',fontsize=12)
ax.grid()

plt.savefig(pltdir+'GALAorNOT_selsens.png')

#%% N5N2 vs N7N2 - tube-transport or not 
region = ['N5N2','N7N2']
loadyears = np.arange(2007,2021)
percentG_df = {}
...
for i,r in enumerate(region):
    ax = fig.add_subplot(gs[i])
    percentG_df[r] = calculate(r, loadyears)[1]
    percentG_df[r].transpose().plot.bar(width=0.7,ax=ax,rot=45,color=color,legend=False)
    
    ax.legend(loc='upper right', bbox_to_anchor=(1.1,1.))
    ax.set_title(f'The fraction of Galapagos parcels that are transported by tube - {r}',fontsize=16)
    ax.set_ylim([0,100])
    ax.set_ylabel('fraction [%]',fontsize=12)
    ax.grid()

ax = fig.add_subplot(gs[2])
improve = percentG_df['N5N2'] - percentT_df['N7N2']
improve.transpose().plot.bar(width=0.7,ax=ax,rot=45,color=color,legend=False)

ax.legend(loc='upper right', bbox_to_anchor=(1.1,1.))
ax.set_title(f'The fraction of Galapagos parcels that are transported by tube',fontsize=16)
ax.set_ylim([-20,80])
ax.set_ylabel('fraction [%]',fontsize=12)
ax.grid()

plt.savefig(pltdir+'PNJorNOT_selsens.png')
...
region = 'PNJtilt'
pltdir = f'/Users/renjiongqiu/Documents_local/Thesis/visualization/quantification/hov/{region}/'
os.makedirs(os.path.dirname(pltdir),exist_ok=True)
# loadyears = np.arange(2007,2010)
# Lats, Lons, Time = calculate(region, loadyears)[-3:]

lats_bin = np.arange(0.5,169.5)
latsline = np.arange(1,169)
Hplt = {}
Time_arr = {}
Lats_arr = {}
timeline = {}
...
for yr in loadyears:
    ...
    Lats_arr[yr] = {}
    for loc in sources:
        # Time_dict = Time[yr]['gala'][loc].values()
        # Lats_dict = Lats[yr]['gala'][loc].values()
        Time_dict = Time[yr][loc].values()
        Lats_dict = Lats[yr][loc].values()
        length = np.shape([*Time_dict])[0]
        arr_T = np.array([],dtype='datetime64[ns]')
        arr_L = np.array([])
        for l in range(length):
            arr_T = np.append(arr_T,[*Time_dict][l])
            arr_L = np.append(arr_L,[*Lats_dict][l])
        Time_arr[yr][loc] = arr_T
        Lats_arr[yr][loc] = arr_L
        
    # load the field data
    secondyear = yr+1
    spd = xr.load_dataset(f'/Users/renjiongqiu/Documents_local/Thesis/visualization/hovmoller/{region}/hovfieldspd_{yr}-{secondyear}PNJtilt_weighted.nc').spdmean
    theta = xr.load_dataset(f'/Users/renjiongqiu/Documents_local/Thesis/visualization/hovmoller/{region}/hovfieldspd_{yr}-{secondyear}PNJtilt_weighted.nc').thetaweight

    mask_field = (theta>160) & (theta<250) & (spd>0.3) # PNJ mask
    
    spd_masked = spd.where(mask_field,0)
    SPD[yr] = spd_masked.data.T
    # if yr%4==0 or (yr+1)%4==0:
    #     timeline[yr] = np.arange(np.datetime64(f'{yr}-01-01','ns'),np.datetime64(f'{yr+1}-12-31','ns'),np.timedelta64(1,'D'))
    timeline[yr] = spd.time_counter.data.astype('datetime64[D]').astype('datetime64[ns]')
    time_bin[yr] = np.append(spd.time_counter.data[0]-np.timedelta64(1,'D'),\
                             spd.time_counter.data)
    # timeline = time[yr].astype('datetime64[ns]')
    ...
    # Hplt[yr]['HC'] = 0
    for loc in sources:
        H, _, _ = np.histogram2d(Lats_arr[yr][loc], Time_arr[yr][loc], bins=[lats_bin,time_bin[yr]])
    ...
    # normalize the nonleap-year size to leap-year size
    if yr%4!=0 and (yr+1)%4!=0:
        SPD[yr] = np.append(SPD[yr],np.zeros((168,1)),axis=1)
        Hplt[yr]['NS'] = np.append(Hplt[yr]['NS'],np.zeros((168,1)),axis=1)
        Hplt[yr]['PB'] = np.append(Hplt[yr]['PB'],np.zeros((168,1)),axis=1)
        # Hplt[yr]['HC'] = np.append(Hplt[yr]['HC'],np.zeros((168,1)),axis=1)
